...
export class DetailedFilesComponent implements OnInit , OnDestroy {
  ...
  fetchItems () {
    ...
    let is_check_all;
    if (this.isReference) {
      is_check_all = this.processDataService.activeProcess.isCheckAll;
      files = this.processDataService.activeProcess.rows;
    }
      this.processService.getFilesList(this.dataTable.criteria, files, is_check_all)
        .then(response => {
